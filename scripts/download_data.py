...
def download_and_extract_dataset(url: str, dataset_name: str, target_dir_name: str):
    ...
    extracted_dirs = [
        d for d in assets_dir.iterdir() if d.is_dir() and d.name.endswith("-master")
    ]
...
if __name__ == "__main__":
    ...
    esc50_response = (
        input("Do you want to proceed with downloading ESC-50? (y/n): ").lower().strip()
    )
    print()
    urbansound8k_response = (
        input("Do you want to proceed with downloading UrbanSound8K? (y/n): ")
        .lower()
        .strip()
    )
    print()
