...
class PatchEmbed(nn.Module):
    def __init__(self, in_chans=1, emb_dim=384, patch_size=16, stride=10):
        super().__init__()
...
class TransformerEncoderBlock(nn.Module):
    def __init__(self, emb_dim=384, num_heads=6, mlp_ratio=4.0, dropout=0.1):
        super().__init__()
...
class ASTViTSmall(nn.Module):
    def __init__(self, sample_rate=44100, patch_size=16, patch_stride=10, overlap=6,
                 num_classes=50, emb_dim=384, depth=12, num_heads=6, f_dim=128):
        super().__init__()
        ...
        # Estimate time dimension for 10s audio (hop_length=160)
        self.t_dim = int((sample_rate * 10) / 160) + 1

        # Patch embedding
        self.patch_embed = PatchEmbed(in_chans=1, emb_dim=emb_dim,
                                      patch_size=patch_size, stride=patch_stride)

        # Grid for positional embedding
        self.grid_size = (
